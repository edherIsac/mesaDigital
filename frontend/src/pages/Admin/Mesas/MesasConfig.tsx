    ...
    <div className="mx-auto max-w-screen-xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Configura y administra las mesas</p>
        </div>
      </div>

      {/* Crear mesa (styled like filters block) */}
      <div className="mb-4 flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03] sm:flex-row">
        <div className="flex-1">
          <Label>Crear mesa</Label>
          {error && (
            <div className="mb-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-400">
              {error}
            </div>
          )}
            ...
            <tbody>
              ...
              {!loading && mesas.length > 0 && mesas.map((m) => (
                <tr key={m.id} className="border-t border-gray-100 transition-colors hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-white/[0.03]">
                  ...
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{m.seats ?? "-"}</td>
                  <td className="px-4 py-3 text-gray-600 dark:text-gray-400">{m.zone ?? "-"}</td>
